...
export class HomeComponent {
  ...
  ngOnInit() {
    ...
    if (logDataStr) {
      ...
      this.showDealerRetailerSelect =
        this.userType != 4 && this.userType !== 13;
  ...
  onTypeChange(type: 'DEALER' | 'RETAILER' | 'SUB_DEALER') {
    if (this.hasCartItems()) {
      this.toastr.warning(
        'Please clear the cart before changing Dealer/Sub Dealer/Retailer',
      );
  ...
  handleReadonlyFocus(event: FocusEvent) {
    if (this.hasCartItems()) {
      ...
      this.toastr.warning('Clear cart before changing value');
    }
  }
